...
export const metadata: Metadata = {
  ...
  description: "Setup, kustomisasi, dan maintenance website WordPress. Paket Starter, Business, Ecommerce.",
};
    ...
    <main className="min-h-screen pt-32 pb-24 bg-background">
      ...
      <section className="container mx-auto px-6 mb-24">
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {packages.map((pkg) => (
            <div key={pkg.name} className={`relative flex flex-col p-8 rounded-3xl border ${pkg.isPopular ? 'border-secondary shadow-xl shadow-secondary/10 bg-card' : 'border-border bg-muted/30'}`}>
              {pkg.isPopular && (
              ...
              <Link
                ...
                className={`w-full inline-flex items-center justify-center whitespace-nowrap rounded-full text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 h-11 px-8 ${pkg.isPopular ? 'bg-secondary text-secondary-foreground hover:bg-secondary/90' : 'border-2 border-border bg-transparent hover:bg-accent hover:text-accent-foreground'}`}
              >
                Pilih Paket
              </Link>
            </div>
